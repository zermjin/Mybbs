...
<%	String id = request.getParameter("id");
	...
	if(id!=null&&!id.equals("")){
		bt = ts.getTopicByID(Integer.parseInt(id));
	}else{
		bt = (BbsTopicEX)session.getAttribute("topic");
	}
	...
	<div class="main layui-clear">
		<div class="fly-panel" pad20>
			...
			<div class="layui-form layui-form-pane">
				<form action="${pageContext.request.contextPath }/EditTopicServlet?oldkiss=${topic.kiss}"
					method="post" onsubmit="return subm(${loginuser.kissNum + topic.kiss});" >
					<div class="layui-form-item">
						<label for="L_title" class="layui-form-label">标题</label>
						<div class="layui-input-block">
							<input type="text" id="L_title" name="title" required value="${topic.title }"
					...
					<div class="layui-form-item layui-form-text">
						<div class="layui-input-block">
							<textarea id="L_content" name="content" required
								...
								class="layui-textarea fly-editor" style="height: 260px;">${topic.content }</textarea>
						</div>
						...
						<div class="layui-inline">
							...
							<div class="layui-input-block">
								<select lay-verify="required" name="category">
									...
									<option value="1" ${topic.categoryId==1? "selected":"" }>提问</option>
									<option value="2" ${topic.categoryId==2? "selected":"" }>分享</option>
									<option value="3" ${topic.categoryId==3? "selected":"" }>讨论</option>
									<option value="4" ${topic.categoryId==4? "selected":"" }>建议</option>
								</select>
							</div>
						</div>
						<div class="layui-inline">
							...
							<div class="layui-input-block">
								<select name="kiss" id="kiss" >
									<option value="5" ${topic.kiss==5? "selected":"" }>5</option>
									<option value="20" ${topic.kiss==20? "selected":"" }>20</option>
									<option value="50" ${topic.kiss==50? "selected":"" }>50</option>
									<option value="100" ${topic.kiss==100? "selected":"" }>100</option>
								</select>
							</div>
						</div>
					</div>
					...
					<div class="layui-form-item">
						<input type="submit" class="layui-btn" lay-filter="*" value="修改完成"></input>
